...
CREATE TABLE IF NOT EXISTS training_records (
    ...
    training_type TEXT NOT NULL CHECK (training_type IN ('初任教育', '定期講習', '事故教育', '特殊教育')), -- 初任教育、定期講習、事故教育、特殊教育は教育訓練の種類
    training_date DATE NOT NULL, -- 教育訓練日
    training_hours REAL, -- 教育訓練時間 
    instructor TEXT, -- 指導員
    notes TEXT, -- 備考
...
CREATE TABLE IF NOT EXISTS health_checkup_records (
    ...
    checkup_type TEXT CHECK (checkup_type IN ('定期', '雇入れ時', '深夜業従事者')),
    overall_result TEXT CHECK (overall_result IN ('異常なし', '要再検査', '要精密検査')),
...
CREATE TABLE IF NOT EXISTS insurance_records (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    employee_id INTEGER NOT NULL REFERENCES employees(id) ON DELETE CASCADE,
    insurance_type TEXT CHECK (insurance_type IN ('健康保険', '厚生年金', '雇用保険', 'マイナンバー')),
...
CREATE TABLE IF NOT EXISTS education_records (
    id INTEGER PRIMARY KEY AUTOINCREMENT,
    employee_id INTEGER NOT NULL REFERENCES employees(id) ON DELETE CASCADE,
    education_type TEXT CHECK (education_type IN ('中学校', '高等学校', '大学', '大学院')),
...
CREATE TABLE IF NOT EXISTS accident_records (
    ...
    accident_type TEXT CHECK (accident_type IN ('車両事故', '人身事故', 'その他')),
...
CREATE TABLE IF NOT EXISTS violation_records (
    ...
    violation_type TEXT CHECK (violation_type IN ('違反', 'その他')),
...
DROP TABLE IF EXISTS health_checkup_records;
