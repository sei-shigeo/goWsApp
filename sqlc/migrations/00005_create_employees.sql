...
CREATE TABLE IF NOT EXISTS employees (
    ...
    gender VARCHAR(20) NOT NULL DEFAULT 'その他' CHECK (gender IN ('男', '女', 'その他')),
    ...
    -- 緊急連絡先
    emergency_contact_name VARCHAR(100), -- 緊急連絡先名前
    emergency_contact_relationship VARCHAR(50), -- 緊急連絡先関係
    emergency_contact_phone VARCHAR(20), -- 緊急連絡先電話番号
    emergency_contact_email VARCHAR(255), -- 緊急連絡先メールアドレス
    emergency_contact_address VARCHAR(500), -- 緊急連絡先住所
    ...
    employment_type_id INTEGER REFERENCES m_employment_types(id) ON DELETE SET NULL, -- 雇用形態
    job_type_id INTEGER REFERENCES m_job_types(id) ON DELETE SET NULL, -- 職種
    department_id INTEGER REFERENCES m_departments(id) ON DELETE SET NULL, -- 部署
    position_id INTEGER REFERENCES m_positions(id) ON DELETE SET NULL, -- 役職
    ...
    -- 在留資格
    nationality_id INTEGER REFERENCES m_nationalities(id) ON DELETE RESTRICT, --　参照先は削除不可
...
DROP TABLE IF EXISTS m_nationalities CASCADE;
DROP TABLE IF EXISTS m_positions CASCADE;
DROP TABLE IF EXISTS m_departments CASCADE;
DROP TABLE IF EXISTS m_employment_types CASCADE;
DROP TABLE IF EXISTS m_job_types CASCADE;
